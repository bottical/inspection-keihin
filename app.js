...
function fetchPickingData() {
    ...
    currentPickingUnsubscribe = currentPickingDocRef.onSnapshot((doc) => {
        if (!doc.exists) {
            playSound('error.mp3', () => { alert("該当するピッキングIDが見つかりませんでした。"); });
        ...
        if (data.status === true && isFirstSnapshot) {
            playSound('error.mp3', () => { alert("このピッキングIDはすでに検品済みです。"); });
            currentPickingId = null;
            currentPickingData = null;
            if (currentPickingUnsubscribe) {
                currentPickingUnsubscribe();
